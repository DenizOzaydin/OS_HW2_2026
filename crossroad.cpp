...
static int max_priority(Direction dir) {
    int max_pr = -1;
    if(dir == DIR_HORIZONTAL) {
        for(int i = 0; i < g_v_lanes; i++) {
            if(g_front_seq_v[i] >= 0 && g_v_pr[i] > max_pr) max_pr = g_v_pr[i];
        }
    } else {
        for(int i = 0; i < g_h_lanes; i++) {
            if(g_front_seq_h[i] >= 0 && g_h_pr[i] > max_pr) max_pr = g_h_pr[i];
        }
    }
    return max_pr;
}

static long long earliest_crossing_seq(Direction dir, int priority) {
    long long earliest = 100000000000000000; 
    if (dir == DIR_HORIZONTAL) {
        for (int i = 0; i < g_v_lanes; i++) {
            if(g_front_seq_v[i] >= 0 && g_v_pr[i] == priority && g_front_seq_v[i] < earliest) earliest = g_front_seq_v[i];
        }
    } else {
        for (int i = 0; i < g_h_lanes; i++) {
            if(g_front_seq_h[i] >= 0 && g_h_pr[i] == priority && g_front_seq_h[i] < earliest) earliest = g_front_seq_h[i];
        }
    }
    return earliest;
}
...
static int can_enter(Direction dir, int lane, long long my_seq) {
    if(dir == DIR_HORIZONTAL) {
        ...
        int my_pr = g_h_pr[lane];
        int max_cross_pr = max_priority(DIR_HORIZONTAL);
        if (max_cross_pr > my_pr) return 0;
        if (max_cross_pr == my_pr) {
            if(earliest_crossing_seq(DIR_HORIZONTAL, my_pr) < my_seq) return 0;
        }
    } else {
        if(any_h_driving()) return 0;
        if(g_drv_v[lane] > 0) return 0;
        int my_pr = g_v_pr[lane];
        int max_cross_pr = max_priority(DIR_VERTICAL);
        if(max_cross_pr > my_pr) return 0;
        if(max_cross_pr == my_pr) {
            if(earliest_crossing_seq(DIR_VERTICAL, my_pr) < my_seq) return 0;
        }
    }
    return 1;
}
...
void initialize_crossroad(int h_lanes, int v_lanes, int* h_pri, int* v_pri) {
    ...
    g_h_pr = (int*)malloc(h_lanes * sizeof(int));
    g_v_pr = (int*)malloc(v_lanes * sizeof(int));
    for (int i = 0; i < h_lanes; i++) g_h_pr[i] = h_pri[i];
    for (int i = 0; i < v_lanes; i++) g_v_pr[i] = v_pri[i];
...
void arrive_crossroad(int car_id, Direction dir, int lane) {
    ...
    if(dir == DIR_HORIZONTAL) {
        ...
        if(my_ticket == g_served_h[lane]) g_front_seq_h[lane] = my_seq;
    } else {
        my_ticket = g_ticket_v[lane]++;
        my_seq = g_arrival_seq++;
        if (my_ticket == g_served_v[lane]) g_front_seq_v[lane] = my_seq;
    }

    while (1) {
        int at_front = dir == DIR_HORIZONTAL ? my_ticket == g_served_h[lane] : my_ticket == g_served_v[lane];
        if (at_front && can_enter(dir, lane, my_seq)) break;

        pthread_cond_wait(&g_cond, &g_mutex);

        if(dir == DIR_HORIZONTAL) {
            if(my_ticket == g_served_h[lane] && g_front_seq_h[lane] < 0) g_front_seq_h[lane] = my_seq;
        } else {
            if(my_ticket == g_served_v[lane] && g_front_seq_v[lane] < 0) g_front_seq_v[lane] = my_seq;
        }
    }
